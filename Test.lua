...
-- example print weight, uuid and pickup proximity prompt of all fruits on farm

-- make sure youre making the most out of intellisense

for _,fruitData:FruitData in GAG:GetFruitsOnFarm() do
    local weight = fruitData.Weight
    local uuid = fruitData.UUID
    local proximityPrompt = fruitData:GetPickupPrompt()
    print(weight, uuid)
    if fruitData:IsWithinPickupRange() then
